...
// Connects several mailboxes at once that all share the same password and
// IMAP host - e.g. a batch of addresses on the same custom domain.
router.post('/bulk', async (req, res) => {
  const { label, provider, pass, host, users } = req.body;
  if (!pass || !provider || !Array.isArray(users) || !users.length) {
    return res.status(400).json({ error: 'provider, pass, and a list of email addresses are required' });
  }
  if (provider === 'custom' && !host) {
    return res.status(400).json({ error: 'IMAP host is required for custom domain mail' });
  }
  const cleanUsers = [...new Set(users.map(u => (u || '').trim()).filter(u => /@/.test(u)))];
  if (!cleanUsers.length) {
    return res.status(400).json({ error: 'No valid email addresses found in the list' });
  }
  if (cleanUsers.length > 60) {
    return res.status(400).json({ error: 'Maximum 60 addresses per bulk add' });
  }

  const db = await readDB();
  const created = cleanUsers.map(user => {
    const account = {
      id: nextId(db, 'account'),
      label: label || user,
      provider,
  ...
  await writeDB(db);
  res.status(201).json(created.map(publicAccount));
});
