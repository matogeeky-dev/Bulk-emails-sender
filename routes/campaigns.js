...
function buildImageLookup(uploadId) {
  const dir = path.join(ZIPS_DIR, uploadId);
  if (!fs.existsSync(dir)) throw new Error('That image upload has expired or was not found - please re-upload the zip');
  const map = new Map();
  for (const filename of fs.readdirSync(dir)) {
    const key = path.parse(filename).name.trim().toLowerCase();
    map.set(key, path.join(dir, filename));
  }
  return map;
}
...
router.post('/', async (req, res) => {
  try {
    const { name, accounts, templates, recipients, startAt, imageZipUploadId } = req.body;
    ...
    let imageLookup;
    if (imageZipUploadId) imageLookup = buildImageLookup(imageZipUploadId);

    const { perAccount, matchedImages } = buildAccountBatches(accounts, cleanRecipients, templates.length, imageLookup);
    ...
    const campaign = {
      ...
      imageZipUploadId: imageZipUploadId || null,
      perAccount
    };

    db.campaigns.push(campaign);
    await writeDB(db);
    res.status(201).json({ ...summarize(campaign), matchedImages: matchedImages || 0 });
  } catch (err) {
