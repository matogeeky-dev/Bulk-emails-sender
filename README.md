...
total recipient count — rotating between up to 20 email copies. Everything
...
2. **New campaign**
   ...
   - Write or paste up to 20 email copies. Use `{{name}}` anywhere you want
...
## Personalized images (one image per recipient)

If every recipient needs a *different* image — e.g. a mockup with their
brand's name on it — upload a zip of images under "3 · Personalized images"
in New Campaign, instead of using the shared attachments above.

**How matching works:** each image's filename (without the extension) must
match a recipient's name exactly, case-insensitive — `Sweet Oven.png`
matches a recipient named "Sweet Oven". After uploading, the form shows how
many recipients matched so you can catch typos before creating drafts.
Unmatched recipients still get their email, just without an image attached.

**Limits:** 3000 images / 500MB zip per upload. Only image files are kept
(`.png .jpg .jpeg .webp .gif`) — anything else in the zip is ignored.

**Important — always start these campaigns immediately, never scheduled for
later.** Unlike the small shared attachments above, personalized-image zips
are far too large to store in the database (500MB vs. MongoDB's 512MB total
free-tier cap), so they're extracted to temporary local disk instead and
deleted once every draft is created. That temporary storage isn't
guaranteed to survive a long wait or a host restart, so the New Campaign
form automatically locks the start time to "now" once you upload a zip.
