...
const state = {
  ...
  imageZipUploadId: null,
  imageZipFilenames: [], // for local match-preview only
};
...
$('#providerSelect').addEventListener('change', (e) => {
  ...
  if (!isCustom) {
    $('#bulkAddToggle').checked = false;
    setBulkMode(false);
  }
});

function setBulkMode(on) {
  $('#bulkEmailField').hidden = !on;
  $('#singleEmailField').hidden = on;
  $('#singleEmailInput').required = !on;
  $('#singleEmailInput').disabled = on;
}

$('#bulkAddToggle').addEventListener('change', (e) => setBulkMode(e.target.checked));
...
$('#accountForm').addEventListener('submit', async (e) => {
  ...
  const isBulk = $('#bulkAddToggle').checked && !$('#bulkAddToggle').closest('.custom-only')?.hidden;

  try {
    if (isBulk) {
      const users = parseEmailList($('#bulkEmailsTextarea').value);
      if (!users.length) {
        msg.textContent = 'Enter at least one valid email address.';
        msg.className = 'form-msg is-error';
        return;
      }
      msg.textContent = `Connecting ${users.length} accounts…`;
      const created = await api('/api/accounts/bulk', {
        method: 'POST',
        body: JSON.stringify({ label: body.label, provider: body.provider, pass: body.pass, host: body.host, users })
      });
      msg.textContent = `Connected ${created.length} accounts.`;
    } else {
...
function mergeRecipients() {
  ...
  $('#totalRecipientsSummary').textContent = combined.length
    ...
    : '';
  renderImageMatchSummary();
  updateLaunchSummary();
}

// ================= NEW CAMPAIGN: PERSONALIZED IMAGES (ZIP) =================
$('#imageZipInput').addEventListener('change', async (e) => {
  const file = e.target.files[0];
  const status = $('#imageZipStatus');
  if (!file) return;
  status.textContent = `Uploading & extracting ${file.name}… this can take a moment for large zips.`;
  status.style.color = '';
  try {
    const fd = new FormData();
    fd.append('zip', file);
    const res = await fetch('/api/image-zips', { method: 'POST', body: fd });
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'Upload failed');
    state.imageZipUploadId = data.uploadId;
    state.imageZipFilenames = data.filenames;
    status.textContent = `${data.fileCount} images extracted from ${file.name}.`;
    // Personalized-image campaigns must run immediately - the extracted
    // files are temporary and aren't guaranteed to survive a long wait.
    $('#startMode').value = 'now';
    $('#startAtField').hidden = true;
    $('#startMode option[value="later"]').disabled = true;
    renderImageMatchSummary();
  } catch (err) {
    status.textContent = err.message;
    status.style.color = 'var(--red)';
    state.imageZipUploadId = null;
    state.imageZipFilenames = [];
    renderImageMatchSummary();
  }
});

function renderImageMatchSummary() {
  const el = $('#imageMatchSummary');
  if (!state.imageZipUploadId) { el.innerHTML = ''; return; }
  const filenameKeys = new Set(state.imageZipFilenames.map(f => f.replace(/\.[^.]+$/, '').trim().toLowerCase()));
  let matched = 0;
  const unmatchedNames = [];
  state.recipients.forEach(r => {
    const key = (r.name || '').trim().toLowerCase();
    if (key && filenameKeys.has(key)) matched++;
    else unmatchedNames.push(r.name || r.email);
  });
  const total = state.recipients.length;
  let html = `<p class="hint" style="color:${matched === total && total ? 'var(--green)' : 'var(--amber)'};">${matched} of ${total} recipients matched an image.</p>`;
  if (unmatchedNames.length && total) {
    const preview = unmatchedNames.slice(0, 8).map(escapeHtml).join(', ');
    const more = unmatchedNames.length > 8 ? ` …and ${unmatchedNames.length - 8} more` : '';
    html += `<p class="hint">No match for: ${preview}${more}</p>`;
  }
  el.innerHTML = html;
}
...
$('#addTemplateBtn').addEventListener('click', () => {
  if (state.templates.length >= 20) { alert('Maximum 20 email copies.'); return; }
...
$('#applyBulkCountBtn').addEventListener('click', () => {
  const value = Number($('#bulkCountInput').value);
  if (!value || value < 1) return;
  const capped = Math.min(value, 100);
  $$('#accountPickerList [data-pick]').forEach(cb => {
    if (!cb.checked) return;
    $(`[data-count="${cb.dataset.pick}"]`).value = capped;
  });
  updateLaunchSummary();
});
...
$('#launchBtn').addEventListener('click', async () => {
  ...
  try {
    const result = await api('/api/campaigns', {
      method: 'POST',
      body: JSON.stringify({
        name, accounts, templates, recipients: state.recipients, startAt,
        imageZipUploadId: state.imageZipUploadId || null
      })
    });
    msg.textContent = state.imageZipUploadId
      ? `Drafts queued. ${result.matchedImages || 0} of ${state.recipients.length} got a matched image.`
      : 'Drafts queued.';
    msg.className = 'form-msg is-ok';
    ...
    $('#imageZipInput').value = '';
    $('#imageZipStatus').textContent = '';
    $('#imageMatchSummary').innerHTML = '';
    $('#startMode option[value="later"]').disabled = false;
    renderTemplates();
    renderAccountPicker();
